...
class SearchTableViewController: UITableViewController, UISearchControllerDelegate, UIAnimatable {
    ...
    private func handleSelection(for symbol: String, searchResult: SearchResult){
        ...
        apiService.fetchTimeSeriesMonthlyAdjustedPublisher(keywords: symbol).sink { [weak self] (completionResult) in
            ...
            self?.hideLoadingAnimation()
            let asset = Asset(searchResult: searchResult, timeSeriesMontlyAdjusted: TimeSeriesMonthlyAdjusted)
            self?.performSegue(withIdentifier: "showCalculator", sender: asset)
            print("succes:  \(TimeSeriesMonthlyAdjusted)")
        }.store(in: &subscribers)
    }
